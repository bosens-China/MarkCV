        ...
        >
          <template #default>
            <div class="flex flex-col gap-2 mt-1">
              <div
                class="text-xs text-gray-500 line-clamp-2 bg-gray-50 p-2 rounded border border-gray-100 font-mono"
              >
              ...
              <n-button
                ...
                secondary
                :type="
                  item.timestamp === activeTimestamp ? 'success' : 'warning'
                "
...
const handleRestore = (item: ResumeHistoryItem) => {
  if (
    window.confirm(
      `确定要恢复到 ${formatDate(item.timestamp)} ${formatTime(item.timestamp)} 的版本吗？当前未保存的内容将丢失。`,
    )
  ) {
    emit('restore', item);
